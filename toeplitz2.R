...
generate_toeplitz <- function(t1, t2, t3) {
  ...
  eigen_values <- eigen(T, only.values = TRUE)$values
  
  # Check if any eigenvalues are non-positive (within tolerance)
  if (any(eigen_values <= 1e-8)) { 
    T <- Matrix::nearPD(T)$mat  # Make it PD
  }
  return(T)
}
...
compute_sure_gbec <- function(X, Y, lambda_seq, Sigma) {
  ...
  if (min(svd_result$d) / max(svd_result$d) < 1e-7) {
    warning("Sigma is nearly singular in SURE. Returning NA for this iteration.")
    return(NA)
  } else {
...
compute_ridge_gbec <- function(X, Y, lambda, Sigma) {
  ...
  # If matrix is close to singular return NA
  if (min(svd_result$d) / max(svd_result$d) < 1e-7) {
    warning("Sigma is nearly singular in Ridge. Returning NA for this iteration.")
    return(NA)
  } 
...
compute_bec <- function(X, Y, tol = 1e-7, Sigma) {
  ...
  if (rcond(Sigma) < tol) {
    warning(paste("Covariance matrix is ill-conditioned, BEC results may be unreliable."))
  }
...
calculate_mse <- function(estimator_func, ..., true_gbec) {
  mse_values <- replicate(num_simulations, {
    ...
    # Calculate the squared error between the estimated and true GBEC score
    (mean(estimated_values, na.rm = TRUE) - true_gbec)^2 
  })
